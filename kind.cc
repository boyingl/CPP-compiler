...
: kind(kind), lexeme(lexeme), replaced(0){}
...
int ASM::Token::toInt() const{
  if(replaced == true){
     ...
     if('-' == lexeme[0]) throw "ERROR: symbol table wrong, address is " + lexeme;
     unsigned long long ul = static_cast<unsigned long long>(l);
     if(ul > 4294967295LL) throw "ERROR: symbol table wrong, address is "+ lexeme;
     return ul;
  }
...
//change token only when it is an operand label
//update the token to corresponding addr
//caller make sure Kind, lexeme valid
void ASM::Token::replaceLabel(const string newLexeme){
  kind = INT;
  lexeme = newLexeme;
  replaced = true;
}
